$(document).ready(function() {
  ...
  $(function() {
    $(window).scroll(function() {
      const scroll = $(window).scrollTop(); // how many pixels scrolled
      const os = $('.mainLogo, .impressum h1').offset().top; // pixels to the top of .mainLogo
      const nav = $('.navbar').height(); // height of .navbar in pixels
  ...
  const nav = $('.navbar').height() + 15;

  $('#e').on('click', function(x) {
    x.preventDefault();
    if (
      window.location.href !== 'http://localhost:3000/datenschutz' &&
      window.location.href !== 'http://localhost:3000/impressum'
    ) {
      $('body, html').animate(
        {
